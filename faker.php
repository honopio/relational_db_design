...
foreach ($coursRows as $coursRow) {
    ...
    for ($i=1; $i <= $howManyChapitre; $i++) {
        ...
        // random number of parties per chapitre, between 1 and 3 per chapitre
        $howManyPartiesPerChapitre = $faker->numberBetween(1, 3);

        for ($j=1; $j <= $howManyPartiesPerChapitre; $j++) {
            $ordreChapitre = $j;  //ordreChapitre auto increments starting from 1
...
/* --------------------------
    ...
    foreach ($utilisateurRows as $utilisateurRow) {
       // Fetch primary key from Utilisateur table
        ...
        // Array to store the courses that have been used, reinitialized for every user
        $usedCourses = array(); 
        ...
        for ($j = 1; $j <= $nbInscriptions; $j++) {
            ...
            if ($faker->boolean(50)) {
                ...
                // 50% chance of commentaireAvis not being null if noteAvis is not null
                if ($faker->boolean(50)) {
...
//Fetch idUtilisateur and numCours from InscriptionCours table
...
foreach ($coursRowsForSession as $coursRowForSession) {
    ...
    if ($faker->boolean(50)) {
        ...
        for ($j = 1; $j <= $nbSessions; $j++) {
            ...
            //if the course has dates, dateHeureDebut and dateHeureFin must be between dateDebut and dateFin
...
foreach ($utilisateurRows as $utilisateurRow) {
    ...
    //a user can't have the same idRole twice. create an array to store the roles that have been used for each user
    $usedRoles = array();

    for ($j = 1; $j <= $nbRoles; $j++) {

        do {
            $Role_idRole = $faker->numberBetween(1, 5); //random idRole between 1 and 5
